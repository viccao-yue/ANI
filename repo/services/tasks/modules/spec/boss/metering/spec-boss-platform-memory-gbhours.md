# SPEC: BOSS platform-memory-gbhours

> Technical specification derived from: `tasks/modules/prd/boss/metering/prd-boss-platform-memory-gbhours.md`  
...
收口 BOSS **平台计量与结算 / 平台 Memory-GBHours** 的权威源事实、平台级技术边界、Console 对照关系与 TODO-YAML 依赖。

### 1.2 PRD Reference

- Source: `tasks/modules/prd/boss/metering/prd-boss-platform-memory-gbhours.md`
- 主维护源: `docs/boss-modules/metering/platform-memory-gbhours.md`
...
|---|---|---|---|---|---|
| GET | `/api/v1/metering/usage` | `—` | `200` | 租户 JWT 上下文 | 单租户只读；`resource_type=instance_memory_gib_seconds` |

**锁定 resource_type**：`instance_memory_gib_seconds`。平台 aggregate — **TODO-YAML** P1。

### 2.3 Non-Frozen Capabilities

- 平台级跨租户 Memory 用量 list/aggregate — **TODO-YAML** P1
...
- 跨租户内存（`instance_memory_gib_seconds`）排行与趋势。
...
见主维护源 `docs/boss-modules/metering/platform-memory-gbhours.md`（§创建前置条件、§操作可用性矩阵）。
...
- `docs/boss-modules/metering/platform-memory-gbhours.md`
