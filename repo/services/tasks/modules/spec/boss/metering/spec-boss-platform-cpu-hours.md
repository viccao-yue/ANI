# SPEC: BOSS platform-cpu-hours

> Technical specification derived from: `tasks/modules/prd/boss/metering/prd-boss-platform-cpu-hours.md`  
...
收口 BOSS **平台计量与结算 / 平台 CPU-Hours** 的权威源事实、平台级技术边界、Console 对照关系与 TODO-YAML 依赖。

### 1.2 PRD Reference

- Source: `tasks/modules/prd/boss/metering/prd-boss-platform-cpu-hours.md`
- 主维护源: `docs/boss-modules/metering/platform-cpu-hours.md`
...
|---|---|---|---|---|---|
| GET | `/api/v1/metering/usage` | `—` | `200` | 租户 JWT 上下文 | 单租户只读；`resource_type=instance_cpu_seconds` |

**锁定 resource_type**：`instance_cpu_seconds`。平台 cross-tenant — **TODO-YAML** P1。

### 2.3 Non-Frozen Capabilities

- 平台级跨租户 CPU 用量 list/aggregate — **TODO-YAML** P1
- BOSS 专属 RBAC scope — 待 Core 确认

### 2.4 Core 同步边界（2026-06-17）

- 平台跨租户 aggregate — **TODO-YAML P1**
- 租户 metering/usage 只读参考，非 BOSS 正式契约
...
- 跨租户 CPU 算力（`instance_cpu_seconds`）排行与趋势。
...
见主维护源 `docs/boss-modules/metering/platform-cpu-hours.md`（§创建前置条件、§操作可用性矩阵）。
...
- `docs/boss-modules/metering/platform-cpu-hours.md`
