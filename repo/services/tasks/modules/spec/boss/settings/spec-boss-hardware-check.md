# SPEC: BOSS hardware-check

> Technical specification derived from: `tasks/modules/prd/boss/settings/prd-boss-hardware-check.md`  
...
收口 BOSS **交付与安装 / 硬件检测** 的权威源事实、平台级技术边界、Console 对照关系与 TODO-YAML 依赖。

### 1.2 PRD Reference

- Source: `tasks/modules/prd/boss/settings/prd-boss-hardware-check.md`
- 主维护源: `docs/boss-modules/settings/hardware-check.md`
...
- 平台级 `硬件检测` 跨租户 list/aggregate API — **TODO-YAML**
- BOSS 专属 RBAC scope — 待 Core/Services 确认
- 见主维护源

### 2.4 Core 同步边界（2026-06-17）

- 硬件检测 REST — **TODO-YAML: N/A**
- CLI preflight 输出为验收依据
- Console **无** 对等页
...
见主维护源 `docs/boss-modules/settings/hardware-check.md`（§创建前置条件、§操作可用性矩阵）。
...
- `docs/boss-modules/settings/hardware-check.md`
