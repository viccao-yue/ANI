# SPEC: BOSS storage-infrastructure

> Technical specification derived from: `tasks/modules/prd/boss/ops/prd-boss-storage-infrastructure.md`  
...
收口 BOSS **资源池与基础设施 / 存储基础设施** 的权威源事实、平台级技术边界、Console 对照关系与 TODO-YAML 依赖。

### 1.2 PRD Reference

- Source: `tasks/modules/prd/boss/ops/prd-boss-storage-infrastructure.md`
- 主维护源: `docs/boss-modules/ops/storage-infrastructure.md`
...
| GET | `/api/v1/metering/usage` | — | 存储 enum **TODO-YAML** |
| GET | `/api/v1/observability/query` | `queryObservability` | PromQL |

平台 storage aggregate — **TODO-YAML P1**

### 2.3 Non-Frozen Capabilities

- 平台 storage infrastructure aggregate — **TODO-YAML** P1
- metering storage `resource_type` — metering 域 **TODO-YAML**

### 2.4 Core 同步边界（2026-06-17）

- `volumes` 为租户卷 API；本页为 **基础设施/backend** 视角
- 禁止跨租户 volumes list 轮询冒充平台 KPI
...
见主维护源 `docs/boss-modules/ops/storage-infrastructure.md`（§创建前置条件、§操作可用性矩阵）。
...
- `docs/boss-modules/ops/storage-infrastructure.md`
