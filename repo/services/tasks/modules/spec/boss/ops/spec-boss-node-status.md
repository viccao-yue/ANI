# SPEC: BOSS node-status

> Technical specification derived from: `tasks/modules/prd/boss/ops/prd-boss-node-status.md`  
...
收口 BOSS **资源池与基础设施 / 节点状态** 的权威源事实、平台级技术边界、Console 对照关系与 TODO-YAML 依赖。

### 1.2 PRD Reference

- Source: `tasks/modules/prd/boss/ops/prd-boss-node-status.md`
- 主维护源: `docs/boss-modules/ops/node-status.md`
...
| GET | `/api/v1/gpu-inventory` | `listGPUInventory` | 节点 GPU 绑定参考 |

平台 `GET /api/v1/nodes` list — **TODO-YAML P1**

### 2.3 Non-Frozen Capabilities

- 平台 nodes list、K8s events — **TODO-YAML** P1
- drain / cordon — Phase 2

### 2.4 Core 同步边界（2026-06-17）

- **无** 独立 nodes list 已冻结路径
- K8s 直连写操作 — Phase 2 Skills
...
见主维护源 `docs/boss-modules/ops/node-status.md`（§创建前置条件、§操作可用性矩阵）。
...
- `docs/boss-modules/ops/node-status.md`
