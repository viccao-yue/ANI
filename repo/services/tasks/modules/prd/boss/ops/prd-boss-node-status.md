...
平台节点与基础设施健康观察页，支持维护标记与 drain 等运维动作入口。
...
- 主维护源：[`docs/boss-modules/ops/node-status.md`](../../docs/boss-modules/ops/node-status.md)
- 当前 v1.yaml **无**独立节点 list 路径；节点视图待 Core 或 observability 扩展。
...
作为平台管理员/SRE，我希望在 BOSS 查看 `节点状态` 的平台级摘要，以便判断全平台运行与风险。
...
- FR-1：必须定义 `节点状态` 的 BOSS 页面定位与非目标
...
- [`docs/boss-modules/ops/node-status.md`](../../docs/boss-modules/ops/node-status.md)
- SPEC：[`tasks/modules/spec/boss/ops/spec-boss-node-status.md`](../../tasks/modules/spec/boss/ops/spec-boss-node-status.md)

## 10. 文档同步状态

- 同步日期：**2026-06-17**
- 详文状态：**满配（Core）** — [`docs/boss-modules/ops/node-status.md`](../../docs/boss-modules/ops/node-status.md)
- SPEC：[`spec-boss-node-status.md`](../spec/spec-boss-node-status.md)
- HTML 摘要：[`ani-services-prototype-boss.html`](../../prototypes/ani-services-prototype-boss.html) · `boss.ops.nodes`
