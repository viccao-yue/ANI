...
`平台 Memory-GBHours` 是 BOSS **平台计量与结算** 域下的 **跨租户内存用量** 专页：按时间范围展示各租户内存（GiB·秒）消耗排行、趋势与占比，供平台运营、SRE、财务做容量规划与配额对账。

本页属于 **Core / Metering** 视角下的 **平台 RBAC** 页面，一级权威源为 `ANI-main/repo/api/openapi/v1.yaml`。

Console [`usage-report.md`](../../console-modules/tenant/usage-report.md) 的「Memory-GBHours」仅为 **当前 JWT 租户** 的 UI 预设视角；本页 **不得** 逐租户切换 JWT 轮询冒充平台大盘。多指标聚合入口见 [`tenant-usage-billing.md`](../tenant/tenant-usage-billing.md)。

## 文档管理规则

- 本文是 `平台 Memory-GBHours` 的主维护源；页面定位、参数口径、字段映射和验收标准以本文为准
- `prototypes/ani-services-prototype-boss.html` 只保留摘要与详文入口
- [`prd-boss-platform-memory-gbhours.md`](../../tasks/modules/prd/boss/metering/prd-boss-platform-memory-gbhours.md) 与 [`spec-boss-platform-memory-gbhours.md`](../../tasks/modules/spec/boss/metering/spec-boss-platform-memory-gbhours.md) 为辅助材料，不替代本文
...
- 本页 UI 名 **Memory-GBHours** 对应 YAML 冻结枚举 `resource_type=instance_memory_gib_seconds`，**不是** 独立 API
...
- 提供 **全平台** 时间范围内的内存（`instance_memory_gib_seconds`）消耗排行与趋势
- 提供按 AZ、day/hour 时间桶的聚合视图（平台 API 合入后）
- 支持从排行行钻取到单租户用量、配额策略（`max_memory_gb`）
- 为 [`tenant-usage-billing.md`](../tenant/tenant-usage-billing.md) 提供 Memory 专页深链
- 为 Phase 2 账单/导出预留边界，当前不写结算金额

## 页面结构

- 首屏至少包含：`时间筛选区`、`平台 KPI 区`、`Memory 趋势图`、`租户排行表`、`边界说明`
- 趋势图与排行表 **共享同一查询上下文**（start/end、group_by）
- 本页锁定 `resource_type=instance_memory_gib_seconds`，不在此页切换至 CPU/GPU/Token 等指标
...
├── KPI：平台 Memory 总消耗、环比、Top N 租户占比
├── 趋势图（group_by=day|hour）
├── 租户排行表（tenant_id / quantity / unit / 占比）
└── 行内钻取 → 租户用量 / quota（max_memory_gb）
```
...
| 层 | 路径 | 本页用法 |
|---|---|---|
| Core | `GET /api/v1/metering/usage` | 租户上下文 **只读参考**；`resource_type=instance_memory_gib_seconds` |
| Core | 平台 metering aggregate **TODO-YAML** | BOSS 正式 list/排行数据源 |
| Services | — | 本页 **不** 承接 Services 计量资源 |

### 关键边界

- 「Memory-GBHours」是 **UI 展示名**；正式枚举为 `instance_memory_gib_seconds`（GiB·秒）
- 单租户 `/metering/usage` **不能** 直接作为 BOSS 跨租户正式契约
- 账单金额、发票、对账不在本页（Phase 2）
- `dev_profile` 为开发/联调辅助字段，**不是** 用户业务展示主字段
- 后端 `unit` 与 UI 展示单位可能不同；UI 换算规则见「字段口径与单位」
...
| 平台 KPI | 平台 aggregate **待 YAML** | 全平台 GiB·秒汇总 | — |
| Memory 趋势图 | 平台 aggregate 或 PromQL 辅助 | `group_by=day\|hour` | 排行表联动 |
| 租户排行表 | 平台 aggregate **待 YAML** | `tenant_id` + `total_quantity` | 租户钻取 |
| 边界说明 | 规划项 | 单租户 API 仅为参考 | tenant-usage-billing |

## BOSS 与 Console 分工

| 维度 | BOSS 平台 Memory-GBHours | Console Memory-GBHours 视角 |
|---|---|---|
| 范围 | 全平台多租户排行 | 单租户 |
| API | 平台 aggregate **待 YAML** | `GET /metering/usage` + UI filter |
| RBAC | 平台 metering 读 | 租户 JWT |
| 钻取 | quota（max_memory_gb）/ 租户 billing | 同页图表+表格 |
...
| 方法 | 路径 | 说明 |
|---|---|---|
| GET | `/api/v1/metering/usage` | 单租户；`resource_type=instance_memory_gib_seconds` → `200` |

| 字段 | 冻结值 |
|---|---|
| `MeteringUsageRecord.resource_type` | `instance_memory_gib_seconds` |
| `group_by`（租户 API） | `resource_type` / `az` / `day` / `hour` |

| 能力 | 状态 |
|---|---|
| 跨租户 Memory 排行 | **TODO-YAML** |
...
| `resource_type` | query | 固定 | `instance_memory_gib_seconds` |
...
| `items[].resource_type` | `MeteringUsageRecord` | 固定 `instance_memory_gib_seconds` |
| `items[].total_quantity` | `MeteringUsageRecord` | 聚合 GiB·秒 |
| `items[].unit` | `MeteringUsageRecord` | 以后端为准；常见 `gib_seconds` 或等价 |
...
| 字段 | 说明 |
|---|---|
| `memory_gbhours_display` | 见「字段口径与单位」换算规则 |
...
| 无数据态 | 「当前时间范围内暂无 Memory 用量」 | **不** 伪造全 0 图表 |
...
| GB·Hours 换算 | 展示层换算，**不** 改 API enum | UI 标签可写 Memory-GBHours |

## 字段口径与单位

| 字段 | 口径 | 单位/格式 |
|---|---|---|
| `resource_type` | 必须为 `instance_memory_gib_seconds` | YAML enum |
| `total_quantity` | 后端聚合原值（GiB·秒） | number (double) |
| `unit` | 以后端为准 | string |
| GB·Hours（UI） | `total_quantity / 3600`（GiB·秒 → GiB·Hours，UI 标签写作 GB·Hours） | 保留 2 位小数 |
| 换算说明 | GiB 与 GB 在 UI 层近似等价展示；**财务对账以 Core 返回 `unit` 为准** | — |
...
|---|---|---|---|---|
| 查看跨租户 Memory 排行 | ✅ | ✅ | ✅ | aggregate 待 YAML |
...
- 本页固定：`resource_type=instance_memory_gib_seconds`
...
- 不得把 UI 名「Memory-GBHours」写入 API `resource_type`
- 图表与排行表必须同参刷新，避免口径漂移
- 平台 aggregate 未上线时，**禁止** 生产环境用逐租户 JWT 轮询作为正式方案
- UI 展示 GB·Hours 时须在边界说明中标注 GiB·秒 → GiB·Hours 换算关系
...
```json
{
  "items": [
    {
      "resource_type": "instance_memory_gib_seconds",
      "total_quantity": 345600,
      "unit": "gib_seconds",
      "period": "day"
    }
  ],
...
```json
{
  ...
  "request_id": "req-boss-mem-400-001"
}
```
...
```json
{
  ...
  "request_id": "req-boss-mem-403-001"
}
```
...
- [`platform-cpu-hours.md`](platform-cpu-hours.md)、[`platform-gpu-hours.md`](platform-gpu-hours.md)
