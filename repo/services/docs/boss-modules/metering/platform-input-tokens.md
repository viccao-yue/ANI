...
`平台 Input Tokens` 是 BOSS **平台计量与结算** 域下的 **跨租户输入 Token 消耗** 专页：按时间范围展示各租户 input token 排行、趋势与占比，供平台运营、财务做推理成本分析与异常租户识别。

本页属于 **Core / Metering** 视角下的 **平台 RBAC** 页面，一级权威源为 `ANI-main/repo/api/openapi/v1.yaml`。

Console [`usage-report.md`](../../console-modules/tenant/usage-report.md) 的「Input Tokens」仅为 **当前 JWT 租户** 的 UI 预设视角；本页 **不得** 逐租户切换 JWT 轮询冒充平台大盘。写入侧 `POST /api/v1/metering/token-usage`（`reportTokenUsage`）由 Services 上报，**不是** 本页查询接口。

## 文档管理规则

- 本文是 `平台 Input Tokens` 的主维护源；页面定位、参数口径、字段映射和验收标准以本文为准
- `prototypes/ani-services-prototype-boss.html` 只保留摘要与详文入口
- [`prd-boss-platform-input-tokens.md`](../../tasks/modules/prd/boss/metering/prd-boss-platform-input-tokens.md) 与 [`spec-boss-platform-input-tokens.md`](../../tasks/modules/spec/boss/metering/spec-boss-platform-input-tokens.md) 为辅助材料，不替代本文
...
- 本页 UI 名 **Input Tokens** 对应 YAML 冻结枚举 `resource_type=token_input`，**不是** 独立 API
...
- 提供 **全平台** 时间范围内的 input token（`token_input`）消耗排行与趋势
- 提供按 day/hour 时间桶的聚合视图（平台 API 合入后）
- 支持从排行行钻取到单租户用量、推理审计
- 为 [`tenant-usage-billing.md`](../tenant/tenant-usage-billing.md) 提供 Input Tokens 专页深链
- 为 Phase 2 账单/导出预留边界，当前不写结算金额

## 页面结构

- 首屏至少包含：`时间筛选区`、`平台 KPI 区`、`Input Token 趋势图`、`租户排行表`、`边界说明`
- 趋势图与排行表 **共享同一查询上下文**（start/end、group_by）
- 本页锁定 `resource_type=token_input`，不与 `token_output` / `token_total` 混页（除非产品改导航）
...
├── KPI：平台 input tokens 总量、环比、Top N 租户占比
├── 趋势图（group_by=day|hour）
├── 租户排行表（tenant_id / quantity / unit / 占比）
└── 行内钻取 → 租户用量 / inference 审计
```
...
| 层 | 路径 | 本页用法 |
|---|---|---|
| Core | `GET /api/v1/metering/usage` | 租户上下文 **只读参考**；`resource_type=token_input` |
| Core | `POST /api/v1/metering/token-usage` | **写入侧**；Services 上报；**非查询** |
| Core | 平台 metering aggregate **TODO-YAML** | BOSS 正式 list/排行数据源 |
| Services | — | 本页 **不** 承接 Services 推理 list 作为 metering 契约 |

### 关键边界

- 「Input Tokens」是 **UI 展示名**；正式枚举为 `token_input`
- `token_total` 在 enum 中存在；本页 **锁定** `token_input`，不与 output/total 混页
- 单租户 `/metering/usage` **不能** 直接作为 BOSS 跨租户正式契约
- `POST /metering/token-usage` 一次上报含 input+output；查询侧按 `resource_type` 拆分
...
| 平台 KPI | 平台 aggregate **待 YAML** | 全平台 input token 汇总 | — |
| Input Token 趋势图 | 平台 aggregate **待 YAML** | `group_by=day\|hour` | 排行表联动 |
| 租户排行表 | 平台 aggregate **待 YAML** | `tenant_id` + `total_quantity` | 租户钻取 |
| 边界说明 | 规划项 | token-usage POST 为写入侧 | platform-output-tokens |

## BOSS 与 Console 分工

| 维度 | BOSS 平台 Input Tokens | Console Input Tokens 视角 |
|---|---|---|
| 范围 | 全平台多租户排行 | 单租户 |
...
| `resource_type` | query | 固定 | `token_input` |
...
| `items[].resource_type` | `MeteringUsageRecord` | 固定 `token_input` |
| `items[].total_quantity` | `MeteringUsageRecord` | 聚合 input token 数 |
...
| 字段 | 说明 |
|---|---|
| `input_tokens_display` | 直接展示 `total_quantity`（当 unit=tokens） |
...
| 无数据态 | 「当前时间范围内暂无 Input Token 用量」 | **不** 伪造全 0 图表 |
...
| 字段 | 口径 | 单位/格式 |
|---|---|---|
| `resource_type` | 必须为 `token_input` | YAML enum |
| `total_quantity` | 后端聚合原值 | number (double) 或 integer |
| `unit` | 以后端为准；常见 `tokens` | string |
| Input Tokens（UI） | 与 `total_quantity` 一致 | 整数或保留 0 位小数 |
...
|---|---|---|---|---|
| 查看跨租户 input token 排行 | ✅ | ✅ | ✅ | aggregate 待 YAML |
...
- 本页固定：`resource_type=token_input`
...
- **不得** 混淆 `token_input` 与 `token_total`（enum 均存在；本页锁定 input）
- 不得把 UI 名「Input Tokens」写入 API `resource_type`
- 图表与排行表必须同参刷新，避免口径漂移
...
```json
{
  ...
  "request_id": "req-boss-input-400-001"
}
```
...
```json
{
  ...
  "request_id": "req-boss-input-403-001"
}
```
...
- [`platform-output-tokens.md`](platform-output-tokens.md)、[`tenant-usage-billing.md`](../tenant/tenant-usage-billing.md)
...
- [x] `resource_type=token_input` 与 enum 一致
- [x] 含响应示例与错误示例
